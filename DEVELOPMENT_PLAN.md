...
- 完成每关独立 NPC 成功反馈：面向玩家、播放成功动画与语音、恢复默认动画和原朝向；反馈与新增星星动画结束后才拆墙、传送玩家和开放下一关。
- 第三关循环已调整为：NPC 保持原朝向正常对话并显示等待 UI；对话结束后播放待机动画、面向玩家并显示回应 UI；回应超时后恢复原朝向并开始下一轮。
- 完成四颗单次运行累计星星、教师完成/取消接口和第四关全部完成事件；四关进度不再写入或读取 `PlayerPrefs`，每次载入场景均从第一关开始。
- `Shift+R` 现在直接重载当前场景；摄像头窗口的位置和尺寸布局仍独立保存。
...
4. 第三关配置循环 NPC、说话/默认动画状态、循环语音、5 秒回应窗口和过早回应提示；`Wait Ui` 拖 NPC 对话阶段图片，`Respond Ui` 拖 NPC 待机并允许回应阶段图片；把未来语音检测或教师按钮连接到 `NotifySpeechDetected()`。
5. 每关各准备一个 `TaskNpcSuccessFeedback`，配置 NPC、默认状态、成功状态、成功语音和持续时间。
6. 挂 `LevelTaskSequenceController`，四项依次拖入三个社交任务和排队任务、四个成功反馈、前三面隔断墙；第一至三项的 `Next Stage Spawn Point` 分别拖入下一关出生位置空物体，第四项可留空；再拖入共享 UI、`StarScoreManager`、模式管理器和玩家。
...
2. 验证第一、二关挥手和预留说话接口均能通关，且当前运行中的完成方式能够区分目标行为与替代行为。
3. 验证第三关 NPC 以原朝向说话并显示等待 UI、过早回应只触发防抖提示；说完后播放待机动画、面向玩家并显示回应 UI，5 秒无回应恢复原朝向重新循环，有效回应进入成功反馈。
4. 验证每关 NPC 面向玩家、成功动画/语音播放、恢复默认状态和原朝向；星星动画结束后对应墙消失，玩家位置和朝向切换到下一关传送点，下一关随后可用。
5. 验证第四关正确/错误位置、20 秒提醒和共享 UI；第四关完成后派发 `onAllTasksCompleted` 且不自动切场景。
6. 普通重新载入场景和按 `Shift+R` 都应验证为：0 颗星、全部隔断墙恢复、只开放第一关；摄像头窗口布局仍保持上次设置。
